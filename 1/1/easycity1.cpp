...
string EasyCity1::solve(string s){
    ...
    query.exec("select cast(round(sum(lat),"+QString::fromStdString(n)+") as char(20)) as sla,cast(round(sum(lon),"+QString::fromStdString(n)+") as char(20)) as slo,(max(length(city))-min(length(city))) as dmm from(select * from (select * from citytable where right(id,1)%2="+QString::fromStdString(evenodd)+") as temp order by length(city) "+QString::fromStdString(ascdec)+",city desc limit 0,"+QString::fromStdString(m)+") as q");
    query.next();
